    ...
    try:
        ...
        try:
            if provider == "novasonic":
                ...
                voice_info = f" (voice: {model_settings['voice_id']})" if 'voice_id' in model_settings else ""
...
Always prefer using tools over generating text responses when tools are available. Keep your voice responses brief and natural."""
                ...
                async def handle_client(websocket, path):
                    ...
                    try:
                        ...
                        receive_task = asyncio.create_task(
                            _receive_from_agent(client_agent, websocket, client_id, server_context)
                        )
                        send_task = asyncio.create_task(
                            _receive_from_browser(client_agent, websocket, client_id, server_context)
                        )
                        
                        # Store tasks for cleanup
                        server_context["clients"][client_id]["tasks"] = [receive_task, send_task]
        ...
        thread = threading.Thread(target=run_server, daemon=False, name=f"voice-chat-{port}")
        thread.start()
...
async def _receive_from_browser(agent, websocket, client_id: str, context: dict) -> None:
...
def _convert_agent_event_to_browser(event: dict, client_context: dict):
    ...
    # Interruption detected - user started speaking (VAD detected)
        ...
        return {
            "type": "interrupted",
            "reason": "voice_activity_detected"
        }
